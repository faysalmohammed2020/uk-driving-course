...
const ExamPage = () => {
  ...
  const totalPages = Math.ceil(questionSets.length / ITEMS_PER_PAGE);
  const currentSets = questionSets.slice(
  ...
  const saveSetChanges = async (setId: number) => {
    ...
    try {
      // Check if this is a new question set or an existing one
      const isNewSet = !questionSets.some((set) => set.id === setId);

      // Send the updated or new question set to the server
      const response = await fetch(`/api/update-exam`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
      ...
      if (isNewSet) {
        ...
        setQuestionSets((prevSets) => [...prevSets, editedSet]);
      } else {
        // Update the existing set
        setQuestionSets((prevSets) =>
          prevSets.map((set) => (set.id === setId ? editedSet : set))
        );
      }
      ...
      <div className="flex justify-center mt-6">
        <button
          onClick={() => {
            ...
            const newId = Math.max(...questionSets.map((set) => set.id), 0) + 1;
            const newSet = {
              id: newId,
            ...
            // Start editing the new set immediately
            setEditingSetId(newId);
            setEditedSet(newSet);
            setSaveError(null);
            setSaveMessage(null);
          }}
          className="px-6 py-3 bg-blue-600 text-white rounded-lg font-semibold hover:bg-blue-700 transition-all duration-300 ease-in-out shadow-md hover:shadow-xl flex items-center gap-2"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            className="h-5 w-5"
          ...
          <div className="bg-white rounded-lg shadow-xl w-full max-w-7xl max-h-[90vh] overflow-hidden flex flex-col">
            <div className="p-4 bg-blue-600 text-white flex justify-between items-center">
              <h3 className="text-xl font-bold">
                {questionSets.some((set) => set.id === editingSetId)
                  ? "Edit"
                  : "Create New"}{" "}
                Question Set
              </h3>
              ...
              <form className="space-y-6">
                ...
                <div className="mb-6">
                  ...
                  <input
                    ...
                    value={editedSet.title}
                    onChange={(e) =>
                      setEditedSet({ ...editedSet, title: e.target.value })
                    }
                    className="w-full text-slate-900 p-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                  />
                </div>
                    ...
                    >
                      <div className="flex justify-between items-start mb-3">
                        ...
                        <button
                          type="button"
                          onClick={() => {
                            const newQuestions = editedSet.questions.filter(
                              (_, i) => i !== qIndex
                            );
                            setEditedSet({
                              ...editedSet,
                                ...
                                <input
                                  ...
                                  onChange={(e) => {
                                    ...
                                    if (e.target.checked) {
                                      ...
                                      newQuestions[qIndex].answer = (
                                        newQuestions[qIndex].answer as string[]
                                      ).filter((a) => a !== option);
                                    }
                                    setEditedSet({
                                      ...editedSet,
